void Pioes_Muoes_Outras_Detetor1(){

TFile *ficheiro = new TFile("AmberTarget_Run_0.root","READ");						                    //Reads a file
       TFile *ficheiroGravar = new TFile("Analise_Pioes_Muoes_1.root","RECREATE");		            	//Saves a file
       TTree *dados = (TTree*) ficheiro->Get("tracksData");						                        //TTree - List of independent columns or branches, represented by the TBranch class
        ...
        dados->Draw("EdepDet1_keV>>Histograma_Pioes","particlePDG==211 || particlePDG==-211");
        Histograma_Pioes->SetFillColor(97);
        Histograma_Pioes->Write();
        Histograma_Pioes->SetTitle("Pioes - Detetor1 ; log(EdepDet1_keV)");
        gPad->SetLogy();
        
        TH1D* Histograma_Muoes = new TH1D("Histograma_Muoes","Histograma_Muoes",nBIns,minBIn,maxBinM);
        janelas[1] = new TCanvas("Muoes", "Muoes");
        dados->Draw("EdepDet1_keV>>Histograma_Muoes","particlePDG==13 || particlePDG==-13");
        Histograma_Muoes->SetFillColor(95);
        Histograma_Muoes->Write();
        Histograma_Muoes->SetTitle("Muoes - Detetor1 ; log(EdepDet1_keV)");
        gPad->SetLogy();
        ...
        dados->Draw("EdepDet1_keV>>Histograma_Outras","particlePDG!=13 || particlePDG!=-13 || particlePDG!=211 || particlePDG!=-211");
        Histograma_Outras->SetFillColor(92);
        Histograma_Outras->Write();
        Histograma_Outras->SetTitle("Outras - Detetor1 ; log(EdepDet1_keV)");
        gPad->SetLogy();
