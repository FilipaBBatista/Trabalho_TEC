void Pioes_Muoes_Outras_Detetor3(){

TFile *ficheiro = new TFile("AmberTarget_Run_0.root","READ");						                    //Reads a file
       TFile *ficheiroGravar = new TFile("Analise_Pioes_Muoes_3.root","RECREATE");		            	//Saves a file
       TTree *dados = (TTree*) ficheiro->Get("tracksData");						                        //TTree - List of independent columns or branches, represented by the TBranch class
        ...
        dados->Draw("EdepDet3_keV>>Histograma_Pioes","particlePDG==211 || particlePDG==-211");
        Histograma_Pioes->SetFillColor(60);
        Histograma_Pioes->Write();
        Histograma_Pioes->SetTitle("Pioes - Detetor3 ; log(EdepDet3_keV)");
        gPad->SetLogy();
        
        TH1D* Histograma_Muoes = new TH1D("Histograma_Muoes","Histograma_Muoes",nBIns,minBIn,maxBinM);
        janelas[1] = new TCanvas("Muoes", "Muoes");
        dados->Draw("EdepDet3_keV>>Histograma_Muoes","particlePDG==13 || particlePDG==-13");
        Histograma_Muoes->SetFillColor(65);
        Histograma_Muoes->Write();
        Histograma_Muoes->SetTitle("Muoes - Detetor3 ; log(EdepDet3_keV)");
        gPad->SetLogy();
        ...
        dados->Draw("EdepDet3_keV>>Histograma_Outras","particlePDG!=13 || particlePDG!=-13 || particlePDG!=211 || particlePDG!=-211");
        Histograma_Outras->SetFillColor(70);
        Histograma_Outras->Write();
        Histograma_Outras->SetTitle("Outras - Detetor3 ; log(EdepDet3_keV)");
        gPad->SetLogy();
